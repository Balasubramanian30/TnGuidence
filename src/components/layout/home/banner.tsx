...
const HomeBanner = () => {
  ...
  const [visible, setVisible] = useState<number[]>(IMAGES.slice(0, 4).map((_, i) => i))
    ...
    <Container className="bg-white !pt-32 h-screen relative overflow-hidden w-full relative">
      ...
      <div className="flex flex-col justify-center items-center-safe h-full gap-7">
        ...
        <Title className="!font-medium leading-tight text-[#5E3B83] space-x-1.5 text-center z-50">
          <span>
            Invest in the Future <br /> with
          </span>
          <span className="bg-gradient-to-r from-[#AB5799] via-[#E870AB] to-[#5E72EB] bg-clip-text text-transparent w-fit text-nowrap">
            Tamil Nadu
          </span>
        </Title>
        ...
        <div className="flex flex-col gap-4 w-fit 4xl:ml-auto z-50">
          <Body className="text-[#3A1F75] !leading-snug max-w-lg xl:!max-w-xl text-center">
            {content?.description ||
              `Tamil Nadu is your gateway to India's industrial excellence — powered by world-class infrastructure, a skilled workforce, and business-forward policies.`}
          </Body>

          <PrimaryButton
            title={content?.cta_button || 'Partner With Us'}
